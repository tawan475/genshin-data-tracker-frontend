...
const scrollToTop = () => {
  if (tableContainer.value) {
    ...
    const scrollParent =
      tableContainer.value.closest('.overflow-auto') || document.documentElement
...
<template>
  <div>
    <div
      ref="tableContainer"
      class="relative overflow-x-auto border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 shadow-sm w-full transition-colors"
    >
      <div
        v-if="isLoading && data.length > 0"
        class="absolute inset-0 z-10 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm flex items-center justify-center transition-colors"
      >
        <span
      ...
      <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
        <thead
          class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 transition-colors"
        >
